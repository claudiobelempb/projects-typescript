...
export const SketetonContent = styled(ContentAtoms)`
  ${({ theme, ...props }) => css`
    ...
    border-radius: 8px;
    cursor: progress;
    background: ${theme.colors[props.$background || 'yellow700']};
    background: linear-gradient(
      90deg,
      ${theme.colors[props.$background || 'gray100']} 0%,
      ${theme.colors[props.$background || 'gray100']} 50%,
      ${theme.colors[props.$background || 'gray100']} 100%
    );
