...
export const BoxDefaultStyle = styled.div<AppType>`
  ${({ theme, ...props }) => css`
    /* z-index: -30; */
    /* height: 100%; */
    position: ${props.appposition && props.appposition};
    /* DISPLAY DEFAULT */
    display: ${props.display && props.display};
    column-gap: ${props.gap?.column && props.gap.column}${props.gap?.unit && props.gap.unit};

    row-gap: ${props.gap?.row && props.gap.row}${props.gap?.unit && props.gap.unit};
