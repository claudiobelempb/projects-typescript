...
export const ButtonAtomsContainer = styled.button<typeDefault>`
  ${({ theme, ...props }) => css`
    border: none;
    width: 100%;
    height: 5rem;
    ...
    &:not(:disabled):hover {
      color: ${theme.variant[props.$variant?.$vcolorHover || 'white']};
      background-color: ${theme.variant[
        props.$variant?.$vbackgroudHover || 'green_100'
      ]};
    }
    ...
    ${props.$font?.$ffamily &&
    css`
      font-family: ${theme.fonts.fontFamily[props.$font.$ffamily]};
    `}
